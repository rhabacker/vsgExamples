...
int main(int argc, char** argv)
{
    ...
    {
        FindMatchingBufferInfo findMatches(geomInfo.positions);
        scene->accept(findMatches);

        std::cout<<"Found matches "<<findMatches.matches.size()<<" for "<<geomInfo.positions<<std::endl;
    ...
    vsg::ref_ptr<vsg::vec3Array> positions = geomInfo.positions;
    ...
    dynamicUpdates = geomInfo.positions.valid() && dynamicUpdates;
